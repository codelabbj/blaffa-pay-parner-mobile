...
export function TransferScreen({ onNavigateBack, onTransactionSuccess }: TransferScreenProps) {
  ...
  useEffect(() => {
    ...
    // Trigger search only if query starts with @
    if (searchQuery.startsWith("@")) {
      const queryContent = searchQuery.slice(1)

      searchTimeoutRef.current = setTimeout(async () => {
        await searchUsers(queryContent)
      }, 300)
    } else {
      setSearchResults([])
  ...
  const searchUsers = async (query: string) => {
    // If query is empty (user just typed @), we still trigger search 
    // to show a list if the backend allows it.
    ...
    >
      ...
      <div className="relative z-10 px-4 pt-safe-area-inset-top pt-12">
        ...
        <div className="flex justify-center mb-8">
          <div className="flex items-center gap-2">
            <div className={`w-8 h-1 rounded-full transition-all duration-300 ${selectedReceiver ? "bg-green-500" : theme === "dark" ? "bg-blue-500" : "bg-blue-600"
              }`}></div>
            <div className={`w-8 h-1 rounded-full transition-all duration-300 ${amount ? "bg-green-500" : theme === "dark" ? "bg-gray-600" : "bg-gray-300"
              }`}></div>
            <div className={`w-8 h-1 rounded-full transition-all duration-300 ${description ? "bg-green-500" : theme === "dark" ? "bg-gray-600" : "bg-gray-300"
              }`}></div>
          </div>
        </div>
      </div>
            ...
            <div className="relative">
              <div className="relative">
                ...
                <Input
                  ...
                  className={`pl-12 h-14 text-lg font-medium rounded-xl border-2 transition-all duration-300 ${theme === "dark"
              ...
              {showSearchResults && searchResults.length > 0 && (
                <div className={`absolute top-full left-0 right-0 mt-2 rounded-xl border shadow-xl z-50 max-h-60 overflow-y-auto ${theme === "dark"
                  ...
                  {searchResults.map((user) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-4 text-left hover:bg-opacity-50 transition-colors ${theme === "dark"
              ...
              {showSearchResults && searchResults.length === 0 && !isSearching && searchQuery.length >= 2 && (
                <div className={`absolute top-full left-0 right-0 mt-2 p-4 rounded-xl border z-50 ${theme === "dark"
                  ? "bg-gray-800/95 border-gray-700/50 backdrop-blur-lg"
                  : "bg-white/95 border-gray-200/50 backdrop-blur-lg"
                  }`}>
              ...
              <div className={`mt-4 p-4 rounded-xl border ${theme === "dark"
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${theme === "dark" ? "bg-green-700" : "bg-green-200"
                      }`}>
                      <UserIcon className="w-5 h-5" />
                    </div>
                    <div>
                  ...
                  <Button
                    ...
                    className={`h-8 w-8 p-0 rounded-lg ${theme === "dark"
          ...
          <div className={`p-5 rounded-2xl border transition-all duration-300 ${theme === "dark"
            ...
            <div className="relative">
              ...
              <Input
                ...
                className={`pl-16 h-16 text-xl font-bold rounded-xl border-2 transition-all duration-300 ${amountError
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-50 p-4 ${theme === "dark"
        ...
        <Button
          ...
          className={`w-full h-14 text-lg font-bold rounded-2xl transition-all duration-200 active:scale-98 ${!amount || !selectedReceiver || !description || isProcessing
